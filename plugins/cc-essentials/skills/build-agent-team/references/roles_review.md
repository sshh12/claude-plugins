...
- A **metric loop** — a number the role drives (research → backtest → refine).
- A **problem loop** — a system or domain the role monitors and fixes (infra issue → diagnose → ship → deploy).
...
Exactly one leader role exists. The leader holds direction, budget, cross-loop arbitration, and institutional memory. Naming varies (CEO, director, captain, lead) — the role matters, not the name.
...
Roles split by closing loops, not by human job specialization. "PM agent / engineer agent / designer agent" is a smell — Claude is multi-domain by default and human functional splits add coordination cost without payoff.

Flag if:
- Role names map directly to human job titles without loop content.
- Two roles split a single loop along skill-specialization lines (e.g., "frontend engineer" + "backend engineer" for one product loop).
...
Flag if:
- A bullet is a virtue ("be thorough", "be careful") without trade-off content.
...
Each critical behavior should map to a failure mode someone can articulate. "Send a message before each session" is a behavior; it should connect to a real failure (the leader losing track of state, conflicting trades) — that connection should be visible in the surrounding text.
...
- Critical behaviors are aspirational ("strive to verify") rather than mandatory ("verify before X").
...
### 11. Synchronization and ownership boundaries

When two roles can interfere with each other's work, the role design must say so explicitly and name the coordination mechanism. The trading-team pattern is a canonical example: infra changes can't ship while the researcher is mid-experiment, and the researcher pauses while infra changes ship. Without an explicit rule, both run in parallel and corrupt each other's state.

Look for:

- **Mutually exclusive work**: Role A's work invalidates Role B's in-progress work, or modifies state Role B is reading. Examples: infra modifying tools the researcher depends on; the trader rebalancing while the researcher is updating signals.
...
### 12. Autonomy boundary is in the role text

The leader's principles or critical behaviors should explicitly state when the leader decides vs. escalates. Default toward "leader decides within constraints; human-only carved narrowly".

Flag if:
- Leader's role text suggests escalating to human is the default for uncertainty.
- No role text addresses what to do when uncertain.
- A non-leader role has authority to escalate to human directly without going through the leader.
